...
function finalizeLead() {
  ...
  // Exibir card de redes sociais após 1.8s
  setTimeout(() => {
    showSocialCard();
  }, 1800);
}
...
/* ── Teclado mobile — sobe o chat junto ──────── */
function setupKeyboardHandler() {
  ...
  function onViewportResize() {
    ...
    if (isSmall) {
      // Altura visível acima do teclado
      const visibleH = vv.height;
      app.style.height = visibleH + 'px';

      // Deslocar para que o topo do viewport virtual seja respeitado
      // (necessário no iOS Safari onde o viewport se move, não redimensiona)
...
function showSocialCard() {
  const card = document.createElement('div');
  card.id = 'social-card';
  card.innerHTML = `
    <div class="social-title">📲 Nos siga nas redes sociais!</div>
    <div class="social-sub">Novidades, lançamentos e promoções exclusivas te esperam.</div>
    <div class="social-btns">
      <a class="social-btn instagram"
         href="https://instagram.com/rfootball.store"
      ...
      <a class="social-btn facebook"
         href="https://facebook.com/rfootballstore"
         target="_blank" rel="noopener noreferrer">
        <svg viewBox="0 0 24 24" fill="currentColor">
  ...
  `;
  // Insere após as mensagens (antes do typing indicator)
  messagesContainer.insertBefore(card, typingIndicator);
  scrollToBottom(true);
}
...
function init() {
  ...
  // Fallback resize para browsers sem visualViewport
